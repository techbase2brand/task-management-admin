...
useEffect(()=>{
  ...
  if(phasejEditObj){
   ...
   console.log(
    phasejEditObj.projectName,"jjjjjjj----ggg"
   );
  ...
  const handleSubmit = () => {
    ...
    if(phasejEditObj){
      ...
      const data = {
        projectName : phasejEditObj.projectName,
        phases: [{...phases[0], phaseName: phasejEditObj.phases}]


      };

      axios
      .put(`http://localhost:5000/api/update-phase/${phasejEditObj.phaseID}`, data)
      .then((response) => {
        console.log(response, "999");
        if (response.data === "OK") {
          navigate("/view-phase");

          // alert(" add phases successfully");
        }
      })
      .catch((error) => {
            ...
            <div className="add-div">
              ...
              {phases.map((phase, index) => (
                <div key={index} style={{ marginTop: "10px" }}>
                  <label className="add-label">Phase {index + 1} Name<span style={{color: "red"}}>*</span></label>
